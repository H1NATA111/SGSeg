...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, guidance_channels, text_guidance_channels, nheads, attention_type, pooling_size, cnext_type="V1", kernel_size=7,
        ...
        self.neck = nn.Conv2d(in_channels*2, in_channels, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, x, text_guidance, guidance=None, corr_guidance=None ,bias=None):
...
class Aggregator(nn.Module):
    def __init__(self, 
        ...
        self.memory_guidance_projection = nn.Sequential(
            nn.Conv2d(appearance_guidance_dim, appearance_guidance_proj_dim, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
        )    
    ...
    def fast_conv_decoder(self, x, text_guidance, guidance, corr_guidance, bias,topK):
        B = x.shape[0]
        T = x.shape[2]
        corr_embed = rearrange(x, 'B C T H W -> (B T) C H W')
        ...
        corr_embed = self.decoder1(corr_embed[aux_valid], text_guidance[0], guidance[0], corr_guidance[0][aux_valid], bias)
        mask_aux0 = self.head1(corr_embed)
        mask_aux0 = rearrange(mask_aux0, '(B T) () H W -> B T H W', B=B)
        aux0_valid = self.get_valid_idx_from_mask(mask_aux0, topK)
        corr_embed = self.decoder2(corr_embed[aux0_valid], text_guidance[1], guidance[1], corr_guidance[1][aux_valid][aux0_valid], bias)
        mask_aux1 = self.head2(corr_embed)
        mask_aux1 = rearrange(mask_aux1, '(B T) () H W -> B T H W', B=B)
        aux1_valid = self.get_valid_idx_from_mask(mask_aux1, topK)
        corr_embed = self.decoder3(corr_embed[aux1_valid], text_guidance[2], guidance[2], corr_guidance[2][aux_valid][aux0_valid][aux1_valid], bias)
        corr_embed = self.head(corr_embed)
        corr_embed = rearrange(corr_embed, '(B T) () H W -> B T H W', B=B)
    ...
    def forward(self, img_feats, text_feats_input, appearance_guidance, logit_scale, gt):
        """
        Arguments:
        ...
        B, T, P, C = text_feats_input.shape
        text_feats_input = text_feats_input.reshape(B*P, T, C)#320,16,768
        pos_text = None #无位置编码
        text_feats_enhence = self.text_enhance_layer(
            src=text_feats_input.transpose(0, 1),
            src_mask=None,  # note we use ~ for mask here  无mask
            src_key_padding_mask=None,#tensor([[False, False, False, False, False, False]], device='cuda:0') 这个根本没用，下级函数里没有调用
            pos=(pos_text.transpose(0, 1) if pos_text is not None else None),#无位置编码，原本的版本中文本编码器为bert，bert的编码生成mask和位置编码
            ).transpose(0, 1)
        
        text_feats = text_feats_enhence.reshape(B, T, P, C)
        ...
        pred = corr.squeeze(1)#2，171，24，24
        if self.__dict__['training'] == True:
            _, gth, gtw = gt.shape
            with torch.no_grad():
                self.memory.update(
                    features=F.interpolate(img_feats, size=(gth, gtw), mode='bilinear', align_corners=False), 
                    segmentation= gt,
                    text= text_feats
                )
        memory_feature = self.memory(img_feats, pred, text_feats)
        ...
        if self.memory_guidance_projection is not None:
            memory_projected_guidance = self.memory_guidance_projection(memory_feature)
        ...
        logit = self.fast_conv_decoder(corr_embed, projected_text_guidance, projected_decoder_guidance, projected_corr_decoder_guidance,memory_projected_guidance, topK=self.topK)
