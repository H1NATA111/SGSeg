...
class Aggregator(nn.Module):
    def __init__(self, 
        ...
        self.pad_len = pad_len
        self.memory = FeaturesMemory(num_classes = 17, feats_channels = appearance_guidance_dim, transform_channels = 128, out_channels = appearance_guidance_dim )
    ...
    def forward(self, img_feats, text_feats, appearance_guidance, gt):
        """
        Arguments:
            img_feats: (B, C, H, W)
            text_feats: (B, T, P, C) 2, 17,1,768
            apperance_guidance: tuple of (B, C, H, W)           0：torch.Size([2, 768, 24, 24]) 1：torch.Size([2, 256, 48, 48]) 2：torch.Size([2, 128, 96, 96])
        """
        corr = self.correlation(img_feats, text_feats)
        classes = None
        B, T, P, C = text_feats.shape
        ...
        bias = self.memory(img_feats).permute(1,0)#1,768
        biases = bias.unsqueeze(0).expand(B, T, 768) 
        text_feats = text_feats.squeeze(2)# B, T, 768

        corss_biases, corss_text_feats = self.feature_fusion_layer(
                        v=biases,
                        l=text_feats,
                        attention_mask_v=None,
                        attention_mask_l=None,
                    )#2,17,768
        corss_biases = F.normalize(biases , dim=2)
        corss_text_feats = F.normalize(text_feats, dim=2)
        text_feats = text_feats + corss_text_feats
        text_feats = text_feats.unsqueeze(2)
        new_visual_feature = corss_biases.mean(dim=1, keepdim=True) #2,1,768
        last_visual_feature = new_visual_feature.squeeze(1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, H, W) #2,768,H,W
        #img_feats = img_feats + last_visual_feature
        appearance_guidance[0] = appearance_guidance[0] + last_visual_feature

        last_visual_feature1 = self.projection1(last_visual_feature) #2,256,H,W
        last_visual_feature1 = F.interpolate(last_visual_feature1, size=(48, 48), mode='bilinear', align_corners=False)
        appearance_guidance[1] = appearance_guidance[1] + last_visual_feature1

        last_visual_feature2 = self.projection2(last_visual_feature) #2,128,H,W
        last_visual_feature2 = F.interpolate(last_visual_feature2, size=(96, 96), mode='bilinear', align_corners=False)
        appearance_guidance[2] = appearance_guidance[2] + last_visual_feature2
