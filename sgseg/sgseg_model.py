...
class SGSeg(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.training:
            self.teacher_sem_seg_head = copy.deepcopy(sem_seg_head)  
        else:
            del self.teacher_sem_seg_head # 
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            self.layers = []
            clip_images_resized = F.interpolate(clip_images.tensor, size=self.clip_resolution, mode='bilinear', align_corners=False)#torch.Size([2, 3, 336, 336]
            if 'eva' in self.clip_name.lower():
                clip_features= self.sem_seg_head.predictor.clip_model.encode_dense(clip_images_resized,
                                                                                        normalize=True,
                                                                                        keep_shape=False,
                                                                                        mode='csa',) # bs, N, C
                clip_features_teacher= self.teacher_sem_seg_head.predictor.clip_model.encode_dense(clip_images_resized,
                                                                                        normalize=True,
                                                                                        keep_shape=False,
                                                                                        mode='csa',) # bs, N, C
            ...
            image_features_student = rearrange(image_features, "B (H W) C -> B C H W", H=24)
            image_features_teacher = rearrange(clip_features_teacher, "B (H W) C -> B C H W", H=24)
            ...
            if self.__dict__['training'] == True:
                gt = torch.stack([x["sem_seg"].to(self.device) for x in batched_inputs], dim=0)
            else:
                gt = None
            outputs = self.sem_seg_head(clip_features, features, gt)
            ...
            loss_rc = self.rc_loss(image_features_student, image_features_teacher)  * 0.1
            losses = {"loss_sem_seg" : loss}
            losses.update({"loss_rc" : loss_rc})
            return losses
        else:
            images = [x["image"].to(self.device) for x in batched_inputs]
            if not self.training and self.sliding_window:
    ...
    def frozen_backbone(self,clip_finetune):
        if 'eva' in self.clip_name.lower():
            for name, params in self.teacher_sem_seg_head.predictor.clip_model.named_parameters():
