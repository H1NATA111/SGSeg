...
from .representation_compensation import  Representation_Compensation
...
class SED(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.sem_seg_head_freeze = sem_seg_head_freeze
    ...
    def from_config(cls, cfg):
        ...
        sem_seg_head_freeze = build_sem_seg_head(cfg, backbone.output_shape())
        
        return {
            "backbone": backbone,
            "sem_seg_head": sem_seg_head,
            "sem_seg_head_freeze": sem_seg_head_freeze,
    ...
    def forward(self, batched_inputs):
        ...
        images = ImageList.from_tensors(images, self.size_divisibility)#torch.Size([4, 3, 768, 768])

        clip_images = F.interpolate(clip_images.tensor, size=self.clip_resolution, mode='bilinear', align_corners=False, )#torch.Size([4, 3, 768, 768])
        clip_features = self.sem_seg_head.predictor.clip_model.encode_image(clip_images, dense=True)##'stem'[4, 192, 192, 192]  res2[4, 192, 192, 192]  res3[4, 384, 96, 96] res4[4, 768, 48, 48] res5[4, 1536, 24, 24]  clip_features['clip_vis_dense'] [4, 768, 24, 24])

        if self.training:
            clip_res3 = clip_features["res3"]
            clip_res4 = clip_features["res4"]
            clip_res5 = clip_features["res5"]
            with torch.no_grad():
                clip_features_freeze = self.sem_seg_head_freeze.predictor.clip_model.encode_image(clip_images, dense=True)##'stem'[4, 192, 192, 192]  res2[4, 192, 192, 192]  res3[4, 384, 96, 96] res4[4, 768, 48, 48] res5[4, 1536, 24, 24]  clip_features['clip_vis_dense'] [4, 768, 24, 24])
            clip_res3_freeze = clip_features_freeze["res3"]
            clip_res4_freeze = clip_features_freeze["res4"]
            clip_res5_freeze = clip_features_freeze["res5"]
            clip_vis_dense_freeze = clip_features_freeze["clip_vis_dense"]
        ...
        clip_vis_dense = clip_features["clip_vis_dense"]#torch.Size([4, 768, 24, 24])
        ...
        if self.training:
            ...
            losses['rc_loss_res3'] = self.rc_loss(clip_res3, clip_res3_freeze)  * 0.1
            losses['rc_loss_res4'] = self.rc_loss(clip_res4, clip_res4_freeze)  * 0.1
            losses['rc_loss_res5'] = self.rc_loss(clip_res5, clip_res5_freeze)  * 0.1
            losses['rc_loss_dense'] = self.rc_loss(clip_vis_dense, clip_vis_dense_freeze)  * 0.1
            return losses
        else:
